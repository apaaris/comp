import numpy, pylab
u = numpy.loadtxt("build/u_wo_bc_fem.txt")
x = numpy.loadtxt("build/x_wo_bc_fem.txt")
pylab.plot(x, u, '-o', label='$u(x)$')
pylab.plot(x, numpy.sin(x), label='$\\sin(x)$')
pylab.legend()
pylab.show()

ubc = numpy.loadtxt("build/u_w_bc_fem.txt")
xbc = numpy.loadtxt("build/x_w_bc_fem.txt")
